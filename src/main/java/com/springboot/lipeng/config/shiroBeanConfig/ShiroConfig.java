...
/**
* @Description:    Shiro配置文件,URL过滤器
* @Author:         LiPeng
* @CreateDate:     2019/8/1 15:20
* @UpdateUser:     LiPeng
* @UpdateDate:     2019/8/1 15:20
* @UpdateRemark:   暂无
* @Version:        1.0
*/
...
//<!-- authc:所有url都必须认证通过才可以访问; anon:所有url都都可以匿名访问-->
        //开放登录接口
//        filterChainDefinitionMap.put("/test/html.do", "anon");
        filterChainDefinitionMap.put("/user/login.do", "anon");
//        filterChainDefinitionMap.put("/test/test.do", "anon");

        // 配置退出过滤器,其中的具体的 退出登录 代码Shiro已经替我们实现了
        filterChainDefinitionMap.put("/logout", "logout");

        List<Permit> permitList = userService.selectPermitByRoleId(0);
        for (Permit permit:permitList) {
            filterChainDefinitionMap.put(permit.getHerf(),"perms["+permit.getPermitName()+"]");
            log.info("========配置的权限列表======="+permit.getHerf()+"----"+"perms["+permit.getPermitName()+"]");
        }
        //游客，开发权限
        filterChainDefinitionMap.put("/guest/**", "anon");
//        //用户，需要角色权限 “user”
//        filterChainDefinitionMap.put("/test/**", "roles[user]");
        filterChainDefinitionMap.put("/test/**", "perms[小小]");
//        //管理员，需要角色权限 “admin”
